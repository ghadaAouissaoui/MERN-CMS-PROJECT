...
exports.login = (req, res, next) => {
  ...
  User.findOne({ email: email, valid: "yes" })
    ...
    .then((match) => {
      ...
      const expireTime = process.env.LOGIN_EXPIRES || 3600000; // 1h par défaut
      ...
      const token = jwt.sign(
        ...
        secret,
        { expiresIn: Math.floor(expireTime / 1000) } // en secondes
      );

      const isProduction = process.env.APPLICATION_START_MODE === "production";
      ...
      const cookieOptions = {
        ...
        domain: "localhost",
        path: "/",   
        sameSite: isProduction ? "None" : "Lax", // "None" si frontend ≠ backend
      };

      // ✅ Log pour déboguer
      console.log("Cookie about to be set with token:", token);

      res.cookie("user_token", token, cookieOptions);
      res.cookie("isLogin", "yes", cookieOptions);
